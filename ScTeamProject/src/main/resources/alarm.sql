...
select * from BS_ALARM where alarmtype=7

delete from BS_ALARM where 
--------------------------------------------------------------------------------------------------------------------------------------------------------------------
/*
...
SELECT BABYNO, ALARMTYPE, COUNT(ALARMTYPE) AS "ALARMAMOUNT", TO_CHAR(ALARMTIME,'YYYY-MM-DD') AS "ALARMTIME",
		CASE WHEN ( SUM((ENDTIME-ALARMTIME)*24*60) gt 60 ) THEN SUM((ENDTIME-ALARMTIME)*24)
			WHEN ( SUM((ENDTIME-ALARMTIME)*24*60) gt 1440 ) THEN SUM((ENDTIME-ALARMTIME))
			ESLE ( SUM((ENDTIME-ALARMTIME)*24*60) )
			END AS "ENDTIME"
FROM BS_ALARM
GROUP BY BABYNO, ALARMTYPE, TO_CHAR(ALARMTIME,'YYYY-MM-DD')
HAVING ALARMTYPE=1 and BABYNO=41 and TO_CHAR(ALARMTIME,'YYYY-MM-DD') > to_char(SYSDATE - 7, 'YYYY-MM-DD')
