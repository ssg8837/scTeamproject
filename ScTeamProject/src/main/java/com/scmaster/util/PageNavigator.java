...
public class PageNavigator {
	//페이지 관련 정보 
	private int countPerPage;		//페이지당 글목록 수-서버에서 가져오기
	private int pagePerGroup;		//그룹당 페이지 수 - 서버에서 가져오기
	private int currentPage;		//현재 페이지 (최근 글이 1부터 시작)-클라이언트 view에서 가져오기 
	private int totalRecordsCount;	//전체 글 수-db에서 가져오기
	...
	public PageNavigator(int countPerPage, int pagePerGroup, int currentPage, int totalRecordsCount) {
		...
		//페이지 숫자를 표현하기 위해서 존재 한다. 페이지 화살표로만 넘기려면 필요없음
		//현재 그룹의 첫 페이지
		...
		//rowbonds 값으로 넘기기 위해서 존재하는 계산식, db에서 select 해 올때 처리속도를 줄이기 위한 계산
